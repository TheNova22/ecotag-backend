...
i2cat = None
with open("data/i2cat_transformer_20210307D3.txt") as f:
    i2cat = f.read().splitlines()
pcmodel = PCModel.load_from_checkpoint("data/transformer_20210307D3.ckpt")
pcmodel.eval()
...
def predictCategory(productName : str) -> list:

    scores = predict(productName, pcmodel, tokenizer)
    top_icats = np.argsort(-scores)[: 10]

    return [i2cat[i] for i in top_icats]
...
if __name__ == "__main__":
    # cli_main()
    print(predictCategory("Lays tasty amazing potato chips 50gm"))